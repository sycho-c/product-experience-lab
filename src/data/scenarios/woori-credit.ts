...
// ─── 13단계 재구성 ─────────────────────────────────────────────────
// 기존 11개 stepActions 사이에 도입부 2개 narrator step 을 추가:
//   01 단체 협업방 운영 소개 (프롤로그)
//   04 비밀 메시지 컨셉 안내 (전환 설명)
const EMPTY_ACTIONS: UIAction[] = [];
const orderedStepActions: UIAction[][] = [
  EMPTY_ACTIONS,        /* 01 단체 협업방 운영 소개 (NEW) */
  stepActions[0],       /* 02 외부 고객 문의 */
  stepActions[1],       /* 03 호스트 답변 + 할 일 등록 */
  EMPTY_ACTIONS,        /* 04 비밀 메시지 컨셉 안내 (NEW) */
  stepActions[2],       /* 05 비밀 메시지 모드 진입 */
  stepActions[3],       /* 06 비밀 메시지 전송 */
  stepActions[4],       /* 07 Guest 시점 전환 */
  stepActions[5],       /* 08 시점 복귀 */
  stepActions[6],       /* 09 비즈폼 메뉴 노출 */
  stepActions[7],       /* 10 비즈폼 모달 열림 */
  stepActions[8],       /* 11 비즈폼 작성 */
  stepActions[9],       /* 12 비즈폼 제출 */
  stepActions[10],      /* 13 할 일 완료 */
];

const stepTitles = [
  '우리금융캐피탈 렌터카 단체 협업방 운영 소개',
  '외부 고객 문의',
  '호스트 답변 + 할 일 등록',
  '정보 분할 — 비밀 메시지 컨셉 안내',
  '비밀 메시지 모드 진입',
...
const stepDescriptions = [
  '우리금융캐피탈 렌터카팀과 다수의 거래처 모집인이 함께 운영하는 단체 대화방입니다. 호스트(렌터카팀)와 여러 외부 거래처가 같은 채널에서 정형화된 문의·여신 한도 검토·승인까지 다대다로 처리합니다. 이미 거래처들이 입장한 상태에서 시작하며, 새로운 문의가 발생하는 흐름을 따라갑니다.',
  '거래처 외부 사용자가 단체 대화방에 한도 상향 문의를 올립니다.',
  '호스트가 답변하면서 메시지에 할 일 chip 을 부착하여 처리를 추적합니다.',
  '단체방은 기본적으로 모든 참여자가 같은 메시지를 봅니다. 하지만 내부 신용 검토 내용처럼 특정 거래처에게만 전달해야 하는 정보가 있을 때 "비밀 메시지" 모드를 사용하면 같은 채널 안에서도 정보 분할 공개가 가능합니다 — 카카오톡으로는 불가능한 시나리오.',
...
const stepActiveSystems: string[][] = [
  ...
  /* 09 비즈폼 메뉴 노출            */ [],
  /* 10 비즈폼 모달 열림            */ [],
  /* 11 비즈폼 작성 (사업자등록증)  */ ['bizVerify'],
  /* 12 비즈폼 제출 → 신용도 검토   */ ['credit', 'limitPolicy'],
  /* 13 할 일 완료 → 한도 적용      */ ['loan'],
];

const stepSystemStatuses: Array<Record<string, string>> = [
  /* 01 */ {},
  /* 02 */ { customer: '한울모빌리티 거래처 조회' },
  /* 03 */ {},
  /* 04 */ {},
  /* 05 */ {},
  /* 06 */ {},
  ...
  /* 09 */ {},
  /* 10 */ {},
  /* 11 */ { bizVerify: '사업자등록증 진위 확인' },
  /* 12 */ { credit: 'A등급 통과', limitPolicy: '1억원 한도 승인 가능' },
  /* 13 */ { loan: '여신 한도 1억원 적용 완료' },
];
