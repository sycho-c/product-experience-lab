...
const stepDescriptions = [
  ...
  '저장하면 사진 메시지 아래에 "처리중" 할 일 chip(메시지에 직접 부착되는 작은 상태 배지) 이 붙고, 우측 RightRail(메인 채팅 옆 보조 패널) 의 할 일 패널에도 같은 항목이 추가됩니다 — 메시지와 할 일을 한 화면에서 같이 추적할 수 있게 됩니다.',
  '할 일을 다시 열어 "고객 등록" 버튼을 누르면 영업 시스템(SFA — Sales Force Automation, 영업 관리 시스템)에 고객이 자동 등록됩니다.',
  '동의서 미수령 상태에서는 "장기 가입 설계" 가 비활성화됩니다. "동의서 요청"을 누르면 설계사 모바일에 가입설계동의서 비즈폼(Cowork+ 내장 양식 — 채팅 안에서 작성·전자서명·제출이 가능한 폼) 이 발송되고, 설계사가 작성·서명·제출하면 채팅에 inline 카드가 추가되며 동의서가 등록완료로 바뀝니다.',
